...
from mmaction.datasets.pipelines.augmentations import Resize, RandomResizedCrop, Flip
...
class DetectionLoad(object):
    ...
    Required keys are "frame_dir", "frame_inds", and "all_detections",      # TODO: remove "frame_dir" in requirements
...
@PIPELINES.register_module()
class ActorCutOut(object):
    """Replace Actor's pixels with mean values.
    ...
    def __call__(self, results):
        """Perform the ``ActorCutOut`` augmentation.
        ...
        for idx in range(len(results['detections'])):
            ...
            if num > 0:
                for i in range(num):
                    box = boxes[i, :]
                    scene_img[box[1]:box[3], box[0]:box[2], :] = self.fill_color
            results['imgs'][idx] = scene_img
